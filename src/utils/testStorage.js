...
export function saveTestAttempt(attempt) {
  ...
  const safeAttempt = {
    ...
    total: Number(attempt.total ?? attempt.questionCount ?? (Array.isArray(attempt.questions) ? attempt.questions.length : 0)),
    ...
    remainingSeconds: Number(attempt.remainingSeconds ?? 0),
